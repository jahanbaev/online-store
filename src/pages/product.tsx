    ...
    useEffect(() => {
        ...
        fetch('https://dummyjson.com/products?limit=100')
            .then(req => req.json()).then(res => {
                if(res.products.filter((product: {id: number}) => product.id === parseInt(getParam("id"))).length === 0){
                    navigate('/404');
                }else{
                    settList(res.products.filter((product: {id: number}) => product.id === parseInt(getParam("id"))));
                    setImage(res.products.filter((product: {id: number}) => product.id === parseInt(getParam("id")))[0].images)
                }
                ...
                return <div className="flex mt-4">
                    ...
                    <div className="w-[100%] mr-8">
                        <div className="h-96  w-[700px] overflow-hidden">
                            <img className=" mb-4 h-96 object-cover  w-[700px] transition-all ease-linear duration-300 hover:scale-150" ref={image} src={product.images[0]} alt="" />
                        </div>
                    ...
                    <div className="w-full max-w-[520px]">
                        <div className="flex">
                            ...
                            <p className="bg-white p-1 pl-2 pr-2 mr-2">{product.category} {">"}</p>
                            <p className="bg-white p-1 pl-2 pr-2 mr-2">{product.brand} {">"}</p>
                            <p className="bg-white p-1 pl-2 pr-2 mr-2 product-title max-w-[180px]">{product.title}</p>
                        </div>
                        <h1 className="text-slate-900 text-3xl mt-8">Product {product.title}</h1>
                        <h1 className="text-blue-700 text-2xl mt-2">price: {product.price} $</h1>
                        <h1 className="text-slate-900 text-xl mt-4 mb-2" >Rating: {product.rating}</h1>
                        <Box
                          sx={{
                            '& > legend': { mt: 2 },
                          }}
                        ...
                        <h1 className="text-slate-900 text-xl mt-4 mb-2" >Stock: {product.stock}</h1>
                        <h1 className="text-slate-900 text-xl mt-4 mb-2" >Discount: {product.discountPercentage}%</h1>
                        <p className="text-slate-900 text-4xl mt-4 mb-2">Description</p>
                        <h1 className="text-slate-900 text-2xl">{product.description}</h1>
                        <div className="bg-white p-2 mt-4 mb-6">
                            <h1 className="text-2xl mb-3 m-1 mt-0">Order now or later for {product.price}$</h1>
                            <div className="flex">
                            <button onClick={()=>show()} className="bg-blue-700 hover:opacity-60 text-white w-full m-1 h-10">Buy now</button>
                            <button onClick={()=>toCart(product.id + "")} className="bg-gray-900 hover:opacity-60 text-white w-full m-1 h-10">{(!cartList.includes(product.id))?"Add to cart": "remove from card"}</button>
                            </div>
                        </div>
                    </div>
                </div>
